...
rm -rf build/vcs/uvm/regression/sva_report build/vcs/uvm/regression/seed_*.vdb \
  ...
  build/vcs/uvm/regression/precoder_out_of_order.vdb
...
echo "UVM AXI-Stream out-of-order completion test"
UVM_TEST=precoder_out_of_order_test VECTORS=2 SEED=$((base_seed + runs + 5)) \
  SVA_COVERAGE=1 SVA_VDB="build/vcs/uvm/regression/precoder_out_of_order.vdb" \
  RUN_LOG="build/vcs/uvm/regression/precoder_out_of_order.log" SKIP_COMPILE=1 \
  bash sim/run_uvm.sh
...
UVM_TEST=precoder_12bit_test VECTORS=1 SEED=$((base_seed + runs + 1)) \
  ...
  RUN_LOG="build/vcs/uvm/regression/precoder_12bit.log" SKIP_COMPILE=1 \
  bash sim/run_uvm.sh
